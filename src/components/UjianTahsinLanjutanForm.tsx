...
import {
  ...
  TahsinPenaltyConfig,
  WaqafSymbolTest,
  DEFAULT_TAHSIN_LANJUTAN_CONFIG,
  WAQAF_SYMBOLS,
  createEmptyTahsinLanjutanEntry,
  createEmptyWaqafTest,
...
export default function UjianTahsinLanjutanForm({
  onSubmit,
  onCancel,
  isPending,
}: Props) {
  ...
  const [config, setConfig] = useState<TahsinPenaltyConfig>({
    ...DEFAULT_TAHSIN_LANJUTAN_CONFIG,
  });
  ...
  const updateEntry = (
    index: number,
    field: keyof TahsinLanjutanEntry,
    value: any
  ) => {
  ...
  const addEntry = () => {
    setEntries([...entries, createEmptyTahsinLanjutanEntry()]);
  };
  ...
  const result = calculateTahsinLanjutanResult(
    entries,
    config,
    penaltiWaqaf,
    waqafTest
    ...
    <div className="bg-card rounded-lg border border-border p-5 shadow-card animate-scale-in space-y-4">
      ...
      <div className="p-4 rounded-lg border border-border bg-muted/40 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="p-3 rounded-md bg-destructive/5 border border-destructive/20">
            ...
            <p className="text-[10px] text-muted-foreground">
              Salah huruf, harakat, tasydid
            </p>
            <p className="text-xs font-bold text-destructive mt-1">
              Penalti: -{config.penalti_lahn_jali} poin
            </p>
          </div>
          ...
          <div className="p-3 rounded-md bg-warning/5 border border-warning/20">
            ...
            <p className="text-xs font-bold text-orange-600 mt-1">
              Penalti: -{config.penalti_lahn_khofi} poin
            </p>
          </div>
          ...
          <div className="p-3 rounded-md bg-accent/50 border border-border">
            ...
            <p className="text-[10px] text-muted-foreground">
              Kesalahan berhenti & memulai
            </p>
            <p className="text-xs font-bold text-foreground mt-1">
              Penalti: -{penaltiWaqaf} poin
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-3 rounded-md bg-background border border-border">
            <div>
              ...
              <input
                ...
                min={0}
                max={10}
                value={config.penalti_lahn_jali}
                onChange={(e) =>
                  setConfig({
                    ...config,
                    penalti_lahn_jali: parseFloat(e.target.value) || 0,
                  })
                }
                className="w-full px-3 py-2 rounded-md border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              />
            ...
            <div>
              ...
              <input
                ...
                min={0}
                max={10}
                value={config.penalti_lahn_khofi}
                onChange={(e) =>
                  setConfig({
                    ...config,
                    penalti_lahn_khofi: parseFloat(e.target.value) || 0,
                  })
                }
                className="w-full px-3 py-2 rounded-md border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              />
            ...
            <div>
              ...
              <input
                ...
                value={penaltiWaqaf}
                onChange={(e) =>
                  setPenaltiWaqaf(parseFloat(e.target.value) || 0)
                }
                className="w-full px-3 py-2 rounded-md border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                min={0}
                max={100}
                value={config.bobot_kelancaran}
                onChange={(e) =>
                  setConfig({
                    ...config,
                    bobot_kelancaran: parseInt(e.target.value) || 0,
                  })
                }
                className="w-full px-3 py-2 rounded-md border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              />
        ...
        <div className="p-3 rounded-md bg-accent/50 border border-border space-y-1">
          <p className="text-xs font-semibold text-foreground">
            Rumus Nilai Per Surat (Tahsin Lanjutan):
          </p>
          <p className="text-xs text-muted-foreground font-mono bg-background/80 px-2 py-1 rounded">
            Koreksi = Kelancaran - (LJ x {config.penalti_lahn_jali}) - (LK x{" "}
            {config.penalti_lahn_khofi}) - (Waqaf x {penaltiWaqaf})
          </p>
          <p className="text-xs font-semibold text-primary font-mono bg-background/80 px-2 py-1 rounded">
            Nilai akhir = Kelancaran - ({config.penalti_lahn_jali} x LJ) - (
            {config.penalti_lahn_khofi} x LK) - ({penaltiWaqaf} x Waqaf &
            Ibtida)
          </p>
        </div>
      </div>
          ...
          >
            ...
            <div className="p-3 rounded-md bg-primary/10 border-2 border-primary/30">
              <h6 className="text-xs font-semibold text-foreground mb-2">
                1. Kelancaran (Prioritas Utama)
              </h6>
              <select
                value={entry.kelancaran}
                onChange={(e) =>
                  updateEntry(index, "kelancaran", parseInt(e.target.value))
                }
                className="w-full px-3 py-2 rounded-md border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              >
              ...
              <p className="text-[10px] text-muted-foreground mt-1">
                Default 90 - dapat diubah oleh penguji
              </p>
            </div>

            <div className="p-3 rounded-md bg-destructive/5 border border-destructive/20">
              <h6 className="text-xs font-semibold text-foreground mb-2">
                2. Lahn Jali (-{config.penalti_lahn_jali}/kesalahan)
              </h6>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { key: "salah_huruf", label: "Salah Huruf" },
                  { key: "salah_harakat", label: "Salah Harakat" },
                  { key: "salah_tasydid", label: "Salah Tasydid" },
                ].map((f) => (
                  <div key={f.key}>
                    ...
                    <input
                      ...
                      min={0}
                      max={50}
                      value={(entry as any)[f.key] ?? 0}
                      onChange={(e) =>
                        updateEntry(
                          index,
                          f.key as keyof TahsinLanjutanEntry,
            ...
            <div className="p-3 rounded-md bg-warning/5 border border-warning/20">
              <h6 className="text-xs font-semibold text-foreground mb-2">
                3. Lahn Khofi (-{config.penalti_lahn_khofi}/kesalahan)
              </h6>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { key: "kesalahan_mad", label: "Mad" },
                  { key: "kesalahan_qalqalah", label: "Qalqalah" },
            ...
            <div className="p-3 rounded-md bg-accent/50 border border-border">
              <h6 className="text-xs font-semibold text-foreground mb-2">
                4. Waqaf & Ibtida (-{penaltiWaqaf}/kesalahan)
              </h6>
              <div>
                <label className="block text-[10px] text-muted-foreground mb-1">
                  Jumlah Kesalahan
            ...
            <div className="p-2 rounded-md bg-muted text-center">
              ...
              <p className="text-xl font-bold text-primary">
                {calculateNilaiTahsinLanjutan(entry, config, penaltiWaqaf)}
              </p>
            </div>
          </div>
        ))}
      ...
      <div className="p-4 rounded-lg border-2 border-violet-500/30 bg-violet-500/5 space-y-3">
          ...
          <h5 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
            ...
            <TooltipProvider delayDuration={200}>
              <Tooltip>
                ...
                <TooltipContent
                  side="top"
                  className="max-w-[280px] whitespace-pre-line text-xs"
                >
                  Centang simbol waqaf yang berhasil dijawab dengan benar oleh
                  siswa.
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
        ...
        <p className="text-[11px] text-muted-foreground">
          Centang simbol yang berhasil dijawab dengan benar dan lancar:
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {WAQAF_SYMBOLS.map((sym) => (
            ...
            >
              <input
                type="checkbox"
                checked={(waqafTest as any)[sym.key]}
                onChange={(e) =>
                  setWaqafTest({
                    ...waqafTest,
                    [sym.key]: e.target.checked,
                  })
                }
        ...
        {!waqafPassed && (
          <p className="text-xs text-destructive font-medium">
            Minimal 3 simbol waqaf harus dijawab benar agar lulus ujian.
          </p>
        )}
      </div>
      ...
      <div
        className={`p-4 rounded-md ${
          result.status === "Lulus" ? "bg-success/10" : "bg-destructive/10"
        }`}
      >
        <div className="flex items-center justify-between">
          <div>
